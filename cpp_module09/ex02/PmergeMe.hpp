...
class PmergeMe
{
    ...
    private:
        template <typename T> void merge_insertion_sort(T& container, int pair_level);
        template <typename T> void swap_pair(T it, int pair_level);
};

long jacobsthal_number(long n);

template <typename T> bool comp(T lv, T rv) { return *lv < *rv; }
...
template <typename T> void PmergeMe::merge_insertion_sort(T& container, int pair_level)
{
    ...
    for (int k = 2;; k++)
    {
        ...
        while (nbr_of_times)
        {
            typename std::vector<Iterator>::iterator idx = std::upper_bound(main.begin(), bound_it, *pend_it, comp<Iterator>);
    ...
    for (ssize_t i = pend.size() - 1; i >= 0; i--)
    {
        ...
        typename std::vector<Iterator>::iterator curr_bound = next(main.begin(), main.size() - pend.size() + i + is_odd);
        typename std::vector<Iterator>::iterator idx = std::upper_bound(main.begin(), curr_bound, *curr_pend, comp<Iterator>);
